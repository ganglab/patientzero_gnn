...
class Custom_dataset(Dataset):
    ...
    def __init__(self, catagory = 'sex', p=0.3, q=0.01, type= 'train' ):
        ...
        for sou,tar,time in data2.values:
            ...
            if sou in node_activation.keys():
                node_activation[ sou ][time] = 1
            else:
                node_activation[ sou ] = np.zeros( self.max_day +1 )
                node_activation[sou][time] = 1

            if tar in node_activation.keys():
                node_activation[tar][time] = 1
            else:
                node_activation[tar] = np.zeros(self.max_day + 1)
                node_activation[tar][time] = 1
        ...
        self.g = nx.Graph()
        self.g.add_edges_from( self.edge_index )

        f1 = open(HOME + '/source/data/{0}_reocrd(p={1},q={2}).txt'.format(catagory, p, q), 'r')
        record = eval(f1.read( ))
        f1.close()
        ...
        print( '{0}_set, sample_num:{1}, edge_feature_shape:{2} node_num:{3} edge_index_shape:{4} catagory:{5} p,q:{6},{7}'.
               format( type, self.sample_num, self.edge_feature.shape, self.nodes_num, self.edge_index.shape, catagory, p, q ) )
