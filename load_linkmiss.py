...
class Custom_dataset(Dataset):
    ...
    def __init__(self, catagory = 'sex', p=0.3, q=0.01, type= 'train', link_miss=0. ):

        self.directed = False if catagory == 'sex' else True
        self.link_miss = link_miss
        ...
        # else:
        # f1 = open(HOME + '/source/data/{0}_reocrd(p={1},q={2})_linkmiss={3}.txt'.
        #                   format(catagory, p, q, link_miss), 'r')
        record = eval(f1.read( ))
        f1.close()
        ...
        print( '{0}_set, sample_num:{1}, edge_feature_shape:{2} node_num:{3} edge_index_shape:{4} link_miss:{5}'.
               format( type, self.sample_num, self.edge_feature.shape,
                       self.nodes_num, self.edge_index.shape, link_miss ) )

    def misslink(self ):
        # np.random.seed(123)
        unmiss_index = np.random.random_sample(self.data.shape[0]) > self.link_miss
        data2 = self.data[unmiss_index]
        ...
        edge_index = np.array(list(edge.keys()))  # [num_edges,2]
        self.edge_index = torch.LongTensor(edge_index).T.to('cuda')
    ...
    def __getitem__(self, index ):
        ...
        node_feature = np.zeros( [self.nodes_num, 3] ).astype( np.float32 ) #[nodes, 3]
        node_feature[:,0] = 1

        eliminate = np.ones( self.nodes_num ).astype( np.float32 ) * Neg #impossible to be source

        for n in infect:
            node_feature[n,:] = [ 0, 1, 0 ]
            eliminate[ n ] = 0
        for n in recovery:
            node_feature[n,:] = [ 0, 0, 1 ]
            eliminate[ n ] = 0
