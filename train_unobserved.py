...
from load_unobserved import  Custom_dataset
...
def Backtracking( catagory = 'sex', p=0.3, q=0.01, test=False, unobserved=0. ):


    train_dataset = Custom_dataset( catagory=catagory, p=p, q=q, type='train', unobserved=unobserved )
    train_loader = DataLoader( dataset=train_dataset, batch_size= batch_size, shuffle=True, num_workers=1,
                               persistent_workers=True )

    BN = gnn.BN( num_node_features=4, num_edge_features=train_dataset.max_day+1 ).cuda() # 3 for SIR, 2 for SI

    op = torch.optim.Adam(BN.parameters(), lr=lr, betas=(0.9, 0.98), eps=1e-9)  # , weight_decay=5e-4 )
    path = HOME + '/source/model_saver/{0}/p={1},q={2},unobserved={3}'.format( catagory, p, q, unobserved )
    ...
    if test == True:
        ...
        test_dataset = Custom_dataset( catagory=catagory, p=p, q=q, type='test', unobserved=unobserved )
        ...
        if unobserved > 0.2:
            path_p = HOME + '/source/model_saver/{0}/p={1},q={2},unobserved={3}'.format(catagory, p, q, round(unobserved-0.2,1) )
    ...
    valid_dataset = Custom_dataset(catagory=catagory, p=p, q=q, type='valid', unobserved= unobserved)
...
if __name__ == '__main__':
    ...
    for cata in ['sex']:
        temp_ = []
        for unobserved in [  0.2, 0.4, 0.6, 0.8 ]:
            # temp = Backtracking( catagory=cata, p=0.8, q=0.05, test=False, unobserved=unobserved )
            temp = Backtracking(catagory=cata, p=0.8, q=0.05, test=True, unobserved=unobserved)
            # temp = Backtracking(catagory='sex', p=0.8, q=0.05, test=False, unobserved=unobserved)
            # temp = Backtracking( catagory = 'sex', p=0.3, q=0.01, test=True, unobserved=unobserved  )
            temp_.append( temp )
        results[cata] = temp_
    print(results)